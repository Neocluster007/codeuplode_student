...
def on_message(client, userdata, message):
    ...
    try:
        ...
        imMainOriginal = Image.open(
            r"/home/pi/MAASProject/Mainimage.png")
        imMainOriginal = imMainOriginal.resize(
            (imMainOriginal.size[0], imMainOriginal.size[1]))

        
        for n in range(len(data['shipments'])):
            #print("ok")
            with open('/home/pi/MAASProject/barcode_ShipmentNumber.jpeg', 'wb') as f:
                Code128(
                    data['shipments'][n]['shipment_number'], writer=ImageWriter()).write(f)

            imBarcode = Image.open(
                r"/home/pi/MAASProject/barcode_ShipmentNumber.jpeg")
            ...
            font = ImageFont.truetype(
                r'/home/pi/MAASProject/browa.ttf', 26)
            font2 = ImageFont.truetype(
                r'/home/pi/MAASProject/browa.ttf', 35)
            font3 = ImageFont.truetype(
                r'/home/pi/MAASProject/browa.ttf', 75)
    ...
    for event in device.read_loop():
        ...
        try:
            ...
            if event.type == evdev.ecodes.EV_KEY:
                ...
                #print(str(data.scancode) + " - " + str(data.keystate) + " - " + str(caps))
                ...
                if data.keystate == 1:
                    if data.scancode == 28:
                        ...
                        if caps:
                            key_lookup = u'{}'.format(capscodes.get(
                                data.scancode)) or u'UNKNOWN:[{}]'.format(data.scancode)
                        else:
                            key_lookup = u'{}'.format(scancodes.get(
                                data.scancode)) or u'UNKNOWN:[{}]'.format(data.scancode)

                        #print("key_lookup ---> " + key_lookup)

                        if (data.scancode != 42 or data.scancode != 28):
                            if(data.scancode != 42):
                                x += key_lookup
                                state_data = True
